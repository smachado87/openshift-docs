...
Puedes crear una nueva aplicacion en OpenShift usando la 
link:../architecture/infrastructure_components/web_console.html[consola web] o
corriendo el comando `oc new-app` command link:#using-the-cli[desde la linea de comando]. Openshift
crea una nueva aplicacion especificandole: link:#specifying-source-code[codigo
fuente], link:#specifying-an-image[imagenes], o link:#specifying-a-template[plantillas].
El comando `new-app` busca imagenes en el directorio local de Docker (si esta disponible),
en un link:../architecture/infrastructure_components/image_registry.html[Registro
Docker], y en el link:../architecture/core_concepts/builds_and_image_streams.html#image-streams[##image stream##]
de OpenShift.

Si se especifica un codigo fuente, `new-app` intenta crear un 
link:builds.html#defining-a-buildconfig[##build configuration##] que desarrolla tu codigo
en una nueva link:../architecture/core_concepts/builds_and_image_streams.html#image-streams[imagen]
de una aplicacion..
